...
export default function Brands() {
	...
	const updateBrandRequest = usePutRequest({url: BRAND_UPDATE.replace('brand_id', brandState.id)})
	const deleteBrandRequest = useDeleteRequest({url: BRAND_DELETE.replace('brand_id', brandState.id)})
	...
	function handleBrandModal () {setbrandModal(false); setbrandState(brandStateInitialValue); setFileList([])}
	function handleDeleteModal() {setdeleteModal(false); setbrandState(brandStateInitialValue)}
	...
	async function handleBrandAddBtn(e){
		...
		if(brandState.name_uz === ''){
			...
			let {success} = await addBrandRequest.request({data: postData(brandState, ['id', 'isNew'])})
			if(success){
	...
	async function handleBrandUpdateBtn (e) {
		...
		if(brandState.name_uz === ''){
			...
			let {success} = await updateBrandRequest.request({data:  postData(brandState, ['id', 'isNew'])})
			if(success){
				brandsListRequest.request()
				setbrandState(brandStateInitialValue)
				setbrandModal(false)
	...
	async function uploadImage (file){
		...
		if(success){
			if(response?.isOk){
				onSuccess(response)
				setbrandState({...brandState, image: response?.image_name})
			}
		}
	}
	...
	return (
		<section className="container">
			...
			<Modal
				...
				centered
				visible={brandModal}
				onOk={(e) => brandState.isNew ?  handleBrandAddBtn(e) : handleBrandUpdateBtn(e)}
			...
			<Row  gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
				{
				 loading ? <FullPageLoader /> :
				   brandList?.map((item, i) =>{
				   	return(
				   		<Col span={8} key={i} style={{marginBottom: '30px'}}>
				   			<Card
				   				cover={
				   					<img
				   						alt="example"
				   						src={item?.image}
				   						style={{width:'70%',height:'150px',padding: '10px', objectFit:'contain', margin: '0 auto'}}
				   					/>
				   				}
				   				actions={[
				   					<Button
				   						style={{width:'100%', height: '100%', border: 'none', boxShadow: 'unset'}}
				   						key="edit"
