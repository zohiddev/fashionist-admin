...
const ProductAdd = () => {
	...
	const productUpdateRequest = usePutRequest({url: PRODUCT_UPDATE.replace('product_id', productData?.id)})
	...
	useEffect(() => {
		if(location?.state?.id !== undefined){
			setLoading(true)
			$authHost.get(PRODUCT_IN_ID.replace('product_id', location?.state?.id)).then(response => {
				if(response?.data?.product){
					...
					setImageState({...imageState, fileList: product?.images?.map(item => {return {url: item}})})
					setAttributeState({
						...attributeState,
						selectedAttributesID: product?.attributes?.map(item => item?.attribute_id),
						selectedAttributesValueID: product?.attributes?.map(item => item?.value_id),
						postAttributeValues: product?.attributes?.map(item => {return {attribute_id: item?.attribute_id, value_id:item?.value_id}})
					})
					setLoading(false)
	...
	useEffect(() => {
		setAttributeState({...attributeState,
			selectedAttributes: attributes?.filter(item => attributeState?.selectedAttributesID?.includes(item.id))
		})
	}, [loading])
	...
	async function uploadImage (file){
		...
		if(success){
			if(response?.isOk){
				onSuccess(response)
				let a = productData.images === '' ? [] : productData.images.split(',')
				console.log(a)
				setProductData({...productData, images: [...a,  ...response?.images].join(',')})
	...
	async function deleteImage (file){
		const {success, response} = await deleteImageRequest.request({data:{image: file.response.images[0]}})
		if(success){
			if(response?.isOk){
				setProductData({...productData, images: productData.images.split(',').filter(el => el !== file.response.images[0]).join(',')})
			}
		}
	}

	async function addProduct (){
		const post_data = postData(productData, ['isNew', 'id','parent_category', 'middle_category','child_category'])
		for(let item in postData(productData, ['previous_price', 'discount', 'quantity', 'is_recommended', 'is_discounted'])){
			if(post_data[item] === initialData[item]){
				return message.warning(`Mahsulot ${productAddedElementsName[item]} kiriting!`)
			}
		}
	...
	async function updateProduct (){
		...
		let {success} = await productUpdateRequest.request({data: post_data})
		if(success) {
			setProductData(initialData)
	...
	function handleCancel () {setImageState({ previewVisible: false })}

	function handleImageChange({ fileList }) {setImageState({ fileList })}
	...
	function changeCategory(arr){
		...
		handleChange({target:{name:'category_id', value:arr[arr.length - 1]}})
	}
		...
		setAttributeState(
			{
				...
				selectedAttributes: attributes.filter(item => arr.includes(item.id))
			})
	}
	...
	function changeAttributeValue(e,mode){
		...
		if(mode === 'delete'){
			...
			for (const item of a) {
				if(item.attribute_id === b.attribute_id){
					a = a.filter(item => item.attribute_id !== b.attribute_id)
				}
			}
			a[a.length] = b
		}
		...
		handleChange({target:{name:'attributes', value: JSON.stringify(a)}})
	}
		...
		<section className="container" >
			<PageHeader
				...
				extra={[
					<Button
						key="2"
						onClick={() => productData?.isNew ? addProduct() : updateProduct()}
					>
						Saqlash
					</Button>]}
			/>
			{
				loading ? <FullPageLoader/> : <Form layout="vertical">
								...
								<TabPane tab="Uz" key='uz'>
									...
									<Form.Item label="Mahsulot tavsifini o`zbekcha kiriting :">
										<ReactQuill
											...
											name='description_uz'
											onChange={(e) => handleChange({target:{name:'description_uz', value:e}})}
										/>
									</Form.Item>
								</TabPane>
								<TabPane tab="Ru" key='ru'>
									...
									<Form.Item label="Mahsulot tavsifini ruscha kiriting :">
										<ReactQuill
											...
											name='description_ru'
											onChange={(e) => handleChange({target:{name:'description_ru', value:e}})}
										/>
									</Form.Item>
								</TabPane>
							</Tabs>
						...
						<TabPane tab="Kategoriyasi" key="2">
							<Form.Item label="Kategoriyani kiriting :">
								<Cascader
									...
									options={categories?.map(item => {return {
										value: item.id,
										label: item.name_uz,
										children:item.children && item.children.map(subitem => {
											return {
												...
												children:subitem.children && subitem.children.map(sublink => {
													return {
														value: sublink.id,
							...
							<Form.Item label="Brandni kiriting :">
								<Select
									...
									onChange={(e) => handleChange({target:{name:'brand_id', value:e}})}
									onSearch={(e) => brandSearch(e)}
								>
									{
										brands?.map(item => {
											return(
												<Option value={item?.id} key={item?.id}>{item?.name_uz}</Option>
											)
										})
									}
						...
						<TabPane tab="Narxi" key="3">
							<Form.Item label="Mahsulot narxini kiriting :">
								<InputNumber
									...
									name='price'
									onChange={(e) => handleChange({target:{name:'price', value:e}})}
									style={{ width: '100%' }}
								/>
							</Form.Item>
							<Form.Item label="Mahsulot sonini kiriting :">
								<InputNumber
									...
									name='quantity'
									onChange={(e) => handleChange({target:{name:'quantity', value:e}})}
									style={{ width: '100%' }}
								/>
							</Form.Item>
							<Form.Item label="Solishtirish uchun narx kiritasizmi ?">
								<Switch
									...
									onChange={() => {setOldPriceActive(!oldPriceActive)}}
								/>
							</Form.Item>
							<Form.Item label="Solishtirish uchun narx kiriting :">
								<InputNumber
									...
									name='previous_price'
									onChange={(e) => handleChange({target:{name:'previous_price', value:e}})}
						...
						<TabPane tab="Attributlari" key="5">
							<Form.Item label="Attributni kiriting :">
								<Select
									...
									optionFilterProp="children"
									onDeselect={(e => changeAttributeValue(e, 'delete'))}
									defaultValue={attributeState.selectedAttributesID}
							...
							<Row gutter={16}>
								{
									attributeState?.selectedAttributes?.map((item,i) => {
										return(
											<Col key={item?.id} span={6}>
												<Form.Item  label={item?.name_uz}>
													<Select
														showSearch
														placeholder="Attributni tanlang"
														defaultValue={JSON.stringify(attributeState?.postAttributeValues[i])}
														optionFilterProp="children"
														onChange={(e) => changeAttributeValue(e, 'add')}
													>
														{
															item?.attributeValues?.map((value, i) => {
																return(
																	<Option
																		value={JSON.stringify({attribute_id: item?.id ,value_id: value?.id })}
																		key={value?.id}
																	>
																		{value?.value_uz}
																	</Option>
