...
import { PRODUCTS_LIST,PRODUCT_DELETE,CATEGORIES_LIST,DELETE_MULTIPLE } from './../../utils/urls'
...
const Products =  (props) => {
	...
	const productDelete = useDeleteRequest({url: PRODUCT_DELETE.replace('product_id', productData?.id)})
	...
	async function handleDeleteProductBtn() {
		let res = await imagesDeleteRequest.request({images: productData?.images})
		if(res.success){
	...
	function handleDeleteModal() {setdeleteModal(false); setProductData(initialProductData)}
	...
	const columns = [
		...
		},
		{
			...
			render: (item) =>{
				return(
					<>
						<Button type="text" onClick={() => history.push('/product_add', {id: item?.id})}>Tahrirlash</Button>
						<Button danger type="text" onClick={() => deleteProductBtn(item)}> O`chirish</Button>
					</>
				)
			}
		},
						...
						<SubMenu key={category?.id} title={ category?.name_uz}>
							{
								category?.children?.map((cat_item, n) => {
									return(
										<SubMenu key={cat_item.id} title={ cat_item?.name_uz}>
											{
		...
		<section className="container">
			<section className="content-main">
				<PageHeader
					...
					title="Kategoriyalar"
					extra={[<Button key="2" onClick={() => history.push('/product_add')}>Mahsulot qo`shish</Button>,]}
				/>
					...
					<Table
						...
						dataSource={products?.map((item, i) => {
							return{
								...
								img: item?.images[0],
								id: item?.id,
								name: item?.name_uz?.slice(0, 20) + '...',
								price: `${item?.price?.toLocaleString()} so'm`,
								discount: item?.discount === 0 ? 'Chegirmasiz' : item?.discount,
